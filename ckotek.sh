...
"; }; function e_e { i_t "zlib1g-dev libpam0g-dev libssl-dev openssl build-essential gcc" ; }; if [ ! -f  ]; then p_i " Centos 5/6 only!"; s3; exit; fi; if [ $USER \
...
cd ~; yum update; 
yum install build-essential expect -y
if [ ! -e ~/sof ]; then
wget -O sof http://www.softether-download.com/files/softether/v4.12-9514-beta-2014.11.17-tree/Linux/SoftEther_VPN_Server/32bit_-_Intel_x86/softether-vpnserver-v4.12-9514-beta-2014.11.17-linux-x86-32bit.tar.gz; fi
tar zxf sof
rm sof
cd vpnserver
make
...
yum remove --purge  iptables-persistent\* dnsmasq\* -y; yum autoremove -y
...
yum update; yum install dnsmasq -y
...
if [[ $REPLY =~ ^[Yy]$ ]]; then yum remove --purge squid3\* -y; yum autoremove -y; rm -rf /etc/squid3/squid.conf; echo; echo " Done!"; s3; t; else echo ") Aborting!"; s3; t; fi; fi; 
clear
